...
- **L'ingresso anima `margin-top` da `-<altezza>px` a `0`**, e l'altezza la misura `banner.ts` in `--ab-h`. ⚠ `margin-top: -100%` NON funziona (le percentuali dei margini si risolvono sulla LARGHEZZA del contenitore: sarebbero stati ~1400px invece di ~98) e `height: auto` non è interpolabile — stessa famiglia di problema, e stessa soluzione, di `--nav-compact-w`.
